...
Communicates with the Anthropic Messages API via httpx.
...
class ClaudeProvider(ILLMProvider):
    """
    Anthropic Claude LLM provider using the Messages REST API.
    ...
    def __init__(self, model_config: Optional[dict] = None):
        ...
        self._model = self._model_config.get("model_name", "claude-haiku-4-5-20251001")
        self._timeout = float(self._model_config.get("request_timeout", 60))

        logger.info(
            f"[ClaudeProvider] Initialized | model={self._model}",
    ...
    async def complete(
        ...
        **kwargs: Any,
        ...
        for attempt in range(_MAX_RETRIES):
            ...
            try:
                logger.debug(
                    f"[ClaudeProvider] Sending request | model={self._model} | "
                ...
                logger.info(
                    f"[ClaudeProvider] Completion SUCCESS | model={self._model} | "
                ...
                logger.warning(
                    f"[ClaudeProvider] HTTP error | status={status} | attempt={attempt + 1} | "
                    f"job_id={job_id} | latency_ms={latency_ms:.1f}",
                    category=LogCategories.LLM_CALL,
                )
                last_exception = ProviderException(
                    message=f"Claude HTTP {status}: {exc.response.text[:200]}",
                    provider_name=self.provider_name,
                    model_name=self._model,
                ...
                last_exception = ProviderException(
                    message=f"Claude connection error: {str(exc)[:100]}",
                    provider_name=self.provider_name,
                    model_name=self._model,
                    retry_count=attempt + 1,
                )
        ...
        logger.error(
            f"[ClaudeProvider] All {_MAX_RETRIES} attempts FAILED | job_id={job_id}",
            category=LogCategories.LLM_CALL,
        )
    ...
    async def health_check(self) -> bool:
        try:
            logger.debug("[ClaudeProvider] Running health check...", category=LogCategories.HEALTH)
            ...
            logger.info(
                f"[ClaudeProvider] Health check: {'HEALTHY' if healthy else 'DEGRADED'}",
